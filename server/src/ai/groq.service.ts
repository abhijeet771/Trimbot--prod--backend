...
export class GroqService implements IAIService {
  ...
  constructor(apiKey: string, modelName: string = 'llama-3.3-70b-versatile') {
    ...
    if (!this.apiKey) {
      logger.warn('Groq API Key is missing. Groq operations will fail.');
    }
  }
  ...
  async generateResponse(
    ...
    try {
      ...
      const groqTools = options?.tools?.map((tool) => ({
        type: 'function',
        function: {
          name: tool.name,
      ...
      logger.info(`Sending chat request to Groq using model ${this.modelName}...`);
      ...
      logger.error(`Groq Service Error: ${errMsg}`);
