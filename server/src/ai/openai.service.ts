...
export class OpenAIService implements IAIService {
  ...
  constructor(apiKey: string, modelName: string = 'gpt-4o') {
    ...
    if (!this.apiKey) {
      logger.warn('OpenAI API Key is missing. OpenAI operations will fail.');
    }
  }
  ...
  async generateResponse(
    ...
    try {
      ...
      logger.info(`Sending chat request to OpenAI using model ${this.modelName}...`);
      ...
      logger.error(`OpenAI Service Error: ${errMsg}`);
